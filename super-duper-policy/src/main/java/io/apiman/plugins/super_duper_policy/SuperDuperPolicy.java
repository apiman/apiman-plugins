 ...
 */
public class SuperDuperPolicy extends AbstractMappedPolicy<SuperDuperConfig> {
     ...
     */
    @Override
    protected Class<SuperDuperConfig> getConfigurationClass() {
        return SuperDuperConfig.class;
    }
     ...
     */
    @Override
    protected void doApply(ApiRequest request, IPolicyContext context, SuperDuperConfig config,
        ...
        if (userRoles == null) {
            String msg = Messages.i18n.format("SuperDuperPolicy.MissingRoles"); //$NON-NLS-1$
        ...
        if (isAuthorized(config, action, userRoles)) {
            ...
            String msg = Messages.i18n.format("SuperDuperPolicy.Unauthorized"); //$NON-NLS-1$
            PolicyFailure failure = context.getComponent(IPolicyFailureFactoryComponent.class).createFailure(
     ...
     * @param userRoles
     */
    private boolean isAuthorized(SuperDuperConfig config, String action, Set<String> userRoles) {
        ...
        for (SuperDuperRule superDuperRule : config.getRules()) {
            boolean actionMatches = "*".equals(superDuperRule.getAction()) || action.equalsIgnoreCase(superDuperRule.getAction()); //$NON-NLS-1$
            if (actionMatches) {
                // the action and resource matched the rule - so enforce the role here!
                boolean userHasRole = userRoles.contains(superDuperRule.getRole());
                matchFound = true;
